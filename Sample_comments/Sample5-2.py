...
with picamera.PiCamera() as camera: #カメラ使用
        ...
        time.sleep(2)
        rawCapture = PiRGBArray(camera,size=(1024,768)) #画像一時保存領域を作成
        for foo in camera.capture_continuous(rawCapture,'bgr',use_video_port=True): #連続撮影開始
            array = foo.array.copy() #コピーを保存
            saitenAndOutput(array) #採点
            rawCapture.truncate(0) #リセット
